...
if (runmode == "MULTICORE") or (runmode == "CLUSTER"):
  for ijob in range(1,ncores+1):
    ...
    this_cmsdrivercmd = cmsdrivercmd+" --python_filename run"+str(ijob)+".py --fileout \"file:"+cwd+"/output/"+sample_name+"/"+nametag+"_"+str(ijob)+".root\" --pileup_input "+minbias_files+" --filein \""+inputlines[ijob-1].strip()+"\""
  ...
  if (year == "2017"):
    os.system("sed -i 's|###BLACKLIST|config.Site.blacklist = ['T2_CH_CERN','T2_KR_KISTI','T2_UK_SGrid_RALPP','T2_IN_TIFR']")
