...
n_minbias = int(610000./nevt_minbias) #JH
...
if (multi_flag == True):
  ...
  for listl in lists:
    ...
    submitmanyshellfile.write("source "+cwd+"/submit_many_dir/submit_"+sample_name+"_second.sh\n")
    os.system("python make_"+nametag+"_second.py "+runmode+" "+sample_name+" "+inputpath)
    inputlinef = open("tmp/"+sample_name+"_second.dat")
  ...
  if (runmode == "CRABJOB"):
    os.system("echo > tmp/"+sample_name+"_second.dat") #JH

if (multi_flag == True):
  sys.exit()
else:
  if os.path.exists("tmp/multi_flag"):
    submitshellfile = open("submit_many_dir/submit_"+sample_name+"_second.sh", "w")
  else:
    submitshellfile = open("submit_second.sh", "w")
...
if (runmode == "CRABJOB"):
  os.system("cp skeleton/crab.py "+runmode+"/"+sample_name+"/crab_second.py")
  os.system("sed -i 's|###REQUESTNAME|config.General.requestName = \"second\"|g' "+runmode+"/"+sample_name+"/crab_second.py")
  os.system("sed -i 's|###PSETNAME|config.JobType.psetName = \""+sample_name+"_second.py\"|g' "+runmode+"/"+sample_name+"/crab_second.py")
  os.system("sed -i 's|###INPUTDATASET|config.Data.inputDataset = \""+inputpath+"\"|g' "+runmode+"/"+sample_name+"/crab_second.py")
  os.system("sed -i 's|###OUTPUTTAG|config.Data.outputDatasetTag = \""+datasettag+"\"|g' "+runmode+"/"+sample_name+"/crab_second.py")
  os.system("sed -i \"s|config.General.workArea = 'crab_projects'||g\" "+runmode+"/"+sample_name+"/crab_second.py")
  os.system("cp skeleton/sedcommand.py "+runmode+"/"+sample_name+"/sedcommand_second.py")
  os.system("sed -i 's|selected_ThisMinBias.dat|selected_ThisMinBias_second.dat|g' "+runmode+"/"+sample_name+"/sedcommand_second.py")
  #os.system("cp skeleton/ThisMinBias.dat "+runmode+"/"+sample_name+"/")

  runshellfile = open(runmode+"/"+sample_name+"/run_second.sh","w")
  this_cmsdrivercmd = cmsdrivercmd+" --python_filename "+sample_name+"_second.py --fileout \""+nametag+".root\" --nThreads 2 --pileup_input "+minbias_files
  runshellfile.write(this_cmsdrivercmd+"\n")
  runshellfile.write("python sedcommand_second.py "+sample_name+"_second.py "+str(n_minbias)+"\n")
  runshellfile.write("crab submit -c crab_second.py\n")
  runshellfile.close()

  submitshellfile.write("cd "+runmode+"/"+sample_name+"\n")
  submitshellfile.write("source run_second.sh\n")
